...
st.subheader("Analisi finanziaria con o senza AI (API OpenAI facoltativa)")
...
"""
        ...
        st.warning(f"GPT non disponibile o API non inserita. Dati generati manualmente. ({e})")
        return {}
...
if uploaded_file:
    ...
    if file_name.endswith(".pdf"):
        ...
        data = extract_data_with_gpt(text, lang, api_key) if api_key else {}
        df = pd.DataFrame(data).T if isinstance(data, dict) and data else pd.DataFrame()
    ...
    st.info("Carica un file per iniziare. L'uso di GPT è facoltativo.")
