...
def parse_financial_data(text, lang):
    ...
    pattern = r"(RICAVI|REVENUE|TOTAL REVENUE|UTILE NETTO|NET INCOME|EBITDA|OPERATING INCOME|CASH FLOW|PATRIMONIO NETTO|EQUITY|TOTALE ATTIVO|TOTAL ASSETS|TOTAL LIABILITIES|COSTI|COSTS|DEBITI|DEBTS)[^\n]*?(€|EUR)?\s*([\d\.,]+)"
    matches = re.findall(pattern, text.upper())
    data = {}

    for match in matches:
        k = match[0].strip().upper()
        v = match[2]
        label = keywords.get(k, k.lower())
        try:
