...
def calculate_kpi(df):
    try: df_out["Equity Ratio (%)"] = df["Equity"] / df["Total Assets"] * 100
    except: pass
    try: df_out["Debt Ratio (%)"] = df["Total Debts"] / df["Total Assets"] * 100
    except: pass
    try: df_out["Asset Turnover"] = df["Revenue"] / df["Total Assets"]
    except: pass
...
if uploaded_file:
    ...
    if not df.empty:
        ...
        st.subheader("📌 Suggerimenti automatici")
        suggestions = []
        if 'Net Margin (%)' in df_kpi.columns and df_kpi['Net Margin (%)'].iloc[0] < 5:
            suggestions.append("📉 Margine netto basso: valuta riduzione costi operativi.")
        if 'Debt to Equity' in df_kpi.columns and df_kpi['Debt to Equity'].iloc[0] > 2:
            suggestions.append("⚠️ Alto rapporto debito/patrimonio netto: potenziale rischio di solvibilità.")
        if 'ROE (%)' in df_kpi.columns and df_kpi['ROE (%)'].iloc[0] < 8:
            suggestions.append("📊 ROE sotto la media: valuta miglioramento redditività.")
        if suggestions:
            for s in suggestions:
                st.markdown(s)
        else:
            st.markdown("✅ Nessun segnale critico rilevato nei KPI.")
